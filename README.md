...
4. 在「画师串预设」中新建并命名风格串，分别填写正面和负面画师串。可导出当前或全部非空预设为 JSON，也可导入别人分享的 JSON；完全重复项会自动跳过，同名但内容不同的预设会自动改名。生成时正面按「正面画师串 → 标签正文 → 模型质量标签」拼接，负面按「负面画师串 → 固定负面提示词」拼接，不会修改聊天原文。
