...
test('NovelAI 按画师串、正文、质量标签的顺序组装提示词', () => {
  ...
  assert.equal(
    composeNovelAiNegativePrompt('artist-specific negative', {
      negativePrompt: 'lowres, bad hands',
    }),
    'artist-specific negative, lowres, bad hands',
  );
});
...
test('NovelAI 站点可填写 Base URL 或完整生图端点', () => {
  ...
  assert.equal(
    normalizeNovelAiEndpoint('https://relay.example/api/generate-direct', '/ai/generate-image'),
    'https://relay.example/api/generate-direct',
  );
});

test('Aurora 常见的 /api 与 /api/v1 地址会自动切换到流式 NAI 路由', () => {
  assert.equal(
...
test('NovelAI V4.5 请求包含官方提示词结构和可复现种子', () => {
  const generated = buildNovelAiPayload({
    ...
    artistNegativePrompt: 'artist-specific negative',
    size: '512x768',
    count: 2,
  });
  ...
  assert.equal(
    generated.body.parameters.v4_negative_prompt.caption.base_caption,
    'artist-specific negative, bad hands',
  );
  assert.equal(generated.resolvedNegativePrompt, 'artist-specific negative, bad hands');
...
test('NAI 中转站使用 Bearer Key、原生请求体并解析 ZIP 响应', async t => {
  ...
  const result = await generateNovelAiImages({
    ...
    artistPrompt: 'artist:sample',
    artistNegativePrompt: 'bad composition',
    prompt: '1girl',
  ...
  assert.equal(request.body.parameters.negative_prompt, 'bad composition');
  assert.equal(result.sources[0].value, PNG_BASE64);
  assert.equal(result.seed, 7);
});

test('Aurora /api 使用 generate-direct 并解析流式相对图片地址', async t => {
  const originalFetch = globalThis.fetch;
  let request;
  globalThis.fetch = async (url, options) => {
    request = { url, options, body: JSON.parse(options.body) };
    return new Response([
      JSON.stringify({ status: 'queued', position: 1, data: '排队中' }),
      JSON.stringify({ status: 'running', data: '生成中' }),
      JSON.stringify({ status: 'success', url: '/img/generated_0.png' }),
      '',
    ].join('\n'), {
      status: 200,
  ...
  const result = await generateNovelAiImages({
    config: {
      ...
      cfgRescale: 0,
      seed: 9,
      negativePrompt: 'global negative',
      qualityTags: false,
      variety: true,
      timeoutMs: 1000,
    },
    apiKey: 'relay-secret-key',
    artistPrompt: 'artist:sample',
    artistNegativePrompt: 'artist negative',
    prompt: '1girl',
    parameters: { size: '512x768', count: 1 },
    settings: { allowHttp: false, maxImageBytes: 1024 * 1024 },
  });

  assert.equal(request.url, 'https://relay.example/api/generate-direct');
  assert.equal(request.options.headers.Authorization, 'Bearer relay-secret-key');
  assert.equal(request.body.parameters.negative_prompt, 'artist negative, global negative');
  assert.deepEqual(result.sources, [{
    sourceType: 'url',
    value: 'https://relay.example/img/generated_0.png',
    generationIndex: 0,
  }]);
  assert.equal(result.resolvedNegativePrompt, 'artist negative, global negative');
});
